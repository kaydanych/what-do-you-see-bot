...
async def _send_welcome(
    ...
    await context.bot.send_message(
        chat_id, t(lang, "WELCOME", name=name, **_times())
    )
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    lang = db.get_user_lang(update.effective_user.id)
    await update.message.reply_text(t(lang, "HELP", **_times()))
...
async def cmd_today(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status, day = day_status()
    if status != "open":
        await update.message.reply_text(t(lang, "NO_ACTIVE_DAY", **_times()))
        return
    prompt = db.get_prompt(day["prompt_id"])
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if status == "none":
        await msg.reply_text(t(lang, "NO_ACTIVE_DAY", **_times()))
        return
    if status == "late":
    ...
    if msg.media_group_id:
        ...
        await msg.reply_text(t(lang, "ACCEPTED", deadline=_times()["deadline"]))


async def on_other(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Text / stickers / video etc. — nudge towards a photo."""
